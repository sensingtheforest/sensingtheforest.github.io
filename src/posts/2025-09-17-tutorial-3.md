---
title: Phase 3 - A DIY Solar-Powered Audio Streamer that Can Record Samples at Solar Times
...
- [Using a Smartphone Hotspot to Connect and Program the Raspberry Pi](#using-a-smartphone-hotspot-to-connect-and-program-the-raspberry-pi)
...
For full details, the code is available in our [GitHub repository](https://github.com/sensingtheforest/audio-streamer), and it's (hopefully!) well-commented. But here's a quick overview of the core structure of the streaming component:
...
We want the RPi to check its inbox regularly, so we set up a cronjob to download the attachments and run `blank.sh` a few seconds after:
...
## Using a Smartphone Hotspot to Connect and Program the Raspberry Pi <a name="using-a-smartphone-hotspot-to-connect-and-program-the-raspberry-pi"></a>

Sometimes (often initially) I had to debug and code the RPi in the forest, and I couldn’t wait an hour for the upload technique through email. It turns out that a smartphone hotspot and a couple of free apps were everything I needed.

<br />

#### 1. Apps, Setup, and Hotspot

1. Download *Net Analyzer* and *Termius* on your smartphone.
2. Make sure you wrote the correct SSID and password of your smartphone hotspot in the appropriate section of `common.sh`.
3. Enable the hotspot and power on the RPi.
...
#### 2. Find the RPi’s IP (Net Analyzer)

1. Connect your phone to its own hotspot.
2. Open *Net Analyzer* → run a LAN scan. (Tip: you may need to turn off your phone’s main Wi-Fi for the scan to succeed.)
3. Look for a device named *Raspberry Pi Foundation* (or similar).
4. Note the IP address (e.g., `192.168.43.101`).
...
#### 3. Connect via SSH (Termius)

1. Open *Termius*.
2. Add a *new host*:

   * Address: IP from Net Analyzer.
   * Username: your RPi username.
   * Password: your RPi password.
3. Save → Connect → You now have an SSH shell to the Pi.
...
1. Coding on a smartphone is a recipe for bugs, but not all smartphones let you connect a laptop and RPi to the hotspot and SSH from the laptop to the RPi. I tried it with a Google Pixel and it worked like a charm, but when students tried with their devices, most blocked this, especially iPhones. Programming directly from the smartphone is often the only solution unless you have a dedicated portable hotspot.

2. **More of a warning → Verify that the RPi auto-connects to your hotspot before heading into the field!**
