...
The streambox tutorial gives you two options: 

1. download their Linux OS, or 
2. build it from scratch following the step by step tutorial. 
...
We'll leave for later the network settings to relay the stream to the [Locus Sonus website](https://locusonus.org/vitae/index.php?page=Locustream.en). For now we want to test the stream's stability, the battery with the solar panels, and the audio hardware. So I'll set up the streamer inside the bird feeder on my window, and when we have the stream going, I'll open a port of my LAN so that we can access the stream from everywhere (NB: opening a port is a security risk).  

The Locus Sonus tutorial uses an old library called DarkIce for the streaming part but BullsEye comes with `ffmpeg` already installed: `ffmpeg` is a very common command line app that can handle the streaming and is widely used for many tasks dealing with AV on Linux.
...
Two little details that could spare you the did-I-spend-one-hour-on-this-for-real moment: 

1. it is http and not https; 
2. the adress box and the search box in a browser are two different things!
...
Shell scripts need executable permission to be used, so we need to run the `chmod` command. `Chmod 777` gives execute permission to everyone. If you want only the owner of the file to be able to execute the script use `755` instead. The and operator `&&` it's only a way to run two command lines together.
