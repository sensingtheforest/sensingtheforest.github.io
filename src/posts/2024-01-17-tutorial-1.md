...
We'll deal later with the network settings to relay the stream to the [Locus Sonus website](https://locusonus.org/vitae/index.php?page=Locustream.en). For now we want to test the stream's stability, the battery with the solar panels, and the audio hardware. So we'll spit the tasks in three steps that will also work as standalone tutorials: 
1. set up the streamer with a common USB mic and use it on my window, inside the house on the LAN;
2. bring the streamer outside and create a bird feeder streamer with stereo mics, so we can see how the equipment fares under various weather conditions;
...
Remember to change my pi username (lu) and local address (192.168.0.102). The dot specifies the home folder on the laptop where the file will be copied.
...
Bash scripts need executable permission to be used, so we need to run the `chmod` command. `Chmod 777` gives execute permission to everyone. If you want only the owner of the file to be able to execute the script use `755` instead. The AND operator `&&` it's only a way to run two command lines together.
...
*   Tutorial 2 - Bird feeder streamer
