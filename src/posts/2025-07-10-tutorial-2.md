...
6. Configure settings:
   ...
   * **Note**: You can keep the originals using `cp` instead of `mv`, only those without `"___o"` will be used. I use mv only to see right away which ones I've already edited
7. Test the stream:
   ```
   ./stream.sh
   ```
...
8. (optional) Enable email operations:
...
9. (optional) Set up cronjobs for automatic problem solving and seding the logs:
   * Open the crontab editor 
   ```
   crontab -e
   ```
   * Copy and paste this content and save.
   ```bash
...
10. (optional) Create boot service to start the streamer when you turn it on. **Don't use the @reboot line in crontab!** That is going to be trouble...
