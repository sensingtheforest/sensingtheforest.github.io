...
System functions and global variables are defined in the `common.sh` script. **`common.sh` also acts as the main user configuration file. Many parameters of the streamer can be adjusted there, and each option is commented directly in the file.**

For audio streaming, we use Darkice, like Locusonus and many other similar projects online.
...
In my case, those extra 12 hours after each reboot gave me time to remotely assign a permanent IP address to the modem, solving the lease expiry problem, sparing me a trip to the site, and, most importantly, avoiding any loss of recordings.
