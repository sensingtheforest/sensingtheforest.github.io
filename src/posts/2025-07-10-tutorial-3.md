...
The battery sensor is enornously useful to monitor a solar power application.
This one is a little crude, but does the job efficiently. I used a very common Analog to Digital Converter to read the voltage directly from the battery, and a voltage divider (literally two resistors) to scale the 12v to 3.3v, a range the RPi pin can read on one of the many input pin on the GPIO.
